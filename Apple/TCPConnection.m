...
{
    @autoreleasepool {
        	[(__bridge TCPConnection*)clientCallBackInfo _handleStreamEvent:type forStream:stream];
    }
}
...
- (BOOL) _writeData:(NSData*)data
{
	...
	header.length = NSSwapHostIntToBig((uint)length);
	...
	while(length > 0) {
		...
		if(result <= 0) {
			REPORT_ERROR(@"Wrote only %i bytes out of %i (%lu) bytes in data", (int)result, (int)length, (unsigned long)[data length]);
			return NO;
		}
		length -= result;
	}
...
- (NSData*) _readData
{
	...
	if(NSSwapBigIntToHost((uint)header.magic) != kMagic) {
		REPORT_ERROR(@"Invalid header", NULL);
		return nil;
	}
	
	length = NSSwapBigIntToHost((uint)header.length);
	...
	while(length > 0) {
		...
		if(result <= 0) {
			REPORT_ERROR(@"Read only %i bytes out of %i (%lu) bytes in data", (int)result, (int)length, (unsigned long)[data length]);
			return nil;
		}
		length -= result;
	}
